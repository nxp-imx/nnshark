 ...
 ******************************************************/
Packet *
packet_new (int cpu_num, int gpu_num, int ddr_num, int pwr_num)
{
  Packet *packet = g_malloc (sizeof (Packet));
  packet->cpu_num = cpu_num;
  packet->gpu_num = gpu_num;
  packet->ddr_num = ddr_num;
  packet->pwr_num = pwr_num;
  ...
  packet->ddr_load = g_malloc0 (sizeof (gfloat) * ddr_num);
  packet->ddr_name = g_malloc0 (sizeof (gchar *) * ddr_num);
  packet->pwr_value = g_malloc0 (sizeof (gfloat) * pwr_num);
  packet->pwr_name = g_malloc0 (sizeof (gchar *) * pwr_num);
  packet->elements =
      g_hash_table_new_full (g_str_hash, g_str_equal, NULL,
...
packet_free (Packet * packet)
{
  ...
  g_free (packet->ddr_load);
  g_free (packet->ddr_name);
  g_free (packet->pwr_value);
  g_free (packet->pwr_name);
